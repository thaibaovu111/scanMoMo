...
@ $zalo = new ZaloPay($ketnoi);
$result = $zalo->LoadData('0334506791')->LoginUser('290503');

echo is_array($result) ? JsonStringFy($result) : $result;
...

{
    ...
    {
        $select = $this->connect->query("SELECT * FROM `zalopay` WHERE `phone` = '".$phone."' ");
        if($select->num_rows == 0){
            $this->connect->query("INSERT INTO `zalopay` SET `phone` = '".$phone."',
                                                             `deviceid` = '".$this->get_device_id()."' ");
                                                               
        }
        $this->config = $this->connect->query("SELECT * FROM `zalopay` WHERE `phone` = '".$phone."' LIMIT 1 ")->fetch_assoc();
        return $this;
    }

    public function LoadData($phone)
    {
        $select = $this->connect->query("SELECT * FROM `zalopay` WHERE `phone` = '".$phone."' LIMIT 1 ");
        if($select->num_rows == 0){
    ...
    public function ImportOTP($code){
        ...
        $this->connect->query("UPDATE `zalopay` SET `phone_verified_token` = '".$result['data']['phone_verified_token']."' WHERE `phone` = '".$this->config['phone']."' ");
            return array(
                'status' => 'success',
    ...
    public function LoginUser($password = ''){
        ...
        $encrypt_pass = hash('sha256', $this->config['password']);
        $encrypt_data = http_build_query(array(
            'pin' => $encrypt_pass
        ));
        $encrypt = $this->RSA_Encrypt($this->config['public_key'],$encrypt_data);
        $pin = $this->CreatPin($encrypt);
        $result = $this->GetDataLogin($pin);

        if(!empty($result['data'])){
            $this->connect->query("UPDATE `zalopay` SET `authorization` = '".$result['data']['session_id']."',
    ...
    public function CheckBeUser()
    {
        ...
        $this->connect->query("UPDATE `zalopay` SET `send_otp_token` = '$send_otp_token' WHERE `phone` = '".$this->config['phone']."' ");
    ...
    public function GetPublicKey()
    {
        ...
        if(!empty($result['data']['public_key'])){
            $this->connect->query("UPDATE `zalopay` SET `public_key` = '".$result['data']['public_key']."' WHERE `phone` = '".$this->config['phone']."' ");
            return array(
                'status' => 'success',
